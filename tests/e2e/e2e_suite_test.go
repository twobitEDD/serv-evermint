...
import (
	...
	"github.com/twobitEDD/servermint/v12/tests/e2e/upgrade"
)

const (
	// defaultManagerNetwork defines the network used by the upgrade manager
	defaultManagerNetwork = "servermint-local"
...
// runNodeWithCurrentChanges builds a docker image using the current branch of the Servermint repository.
...
func (s *IntegrationTestSuite) proposeUpgrade(name, target string) {
	...
	// if Servermint is based on Evmos lower than v10.x.x no need to use the legacy proposal
