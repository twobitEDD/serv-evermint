...
// ServermintVersion is a custom comparator for sorting semver version strings.
type ServermintVersion []string

// Len is the number of stored versions..
func (v ServermintVersion) Len() int { return len(v) }

// Swap swaps the elements with indexes i and j. It is needed to sort the slice.
func (v ServermintVersion) Swap(i, j int) { v[i], v[j] = v[j], v[i] }

// Less compares semver versions strings properly
func (v ServermintVersion) Less(i, j int) bool {
...
func CheckLegacyProposal(version string) bool {
	...
	cmp := ServermintVersion([]string{version, "v10.0.0"})
...
func RetrieveUpgradesList(upgradesPath string) ([]string, error) {
	...
	sort.Sort(ServermintVersion(versions))
