...
// NewCoordinator initializes Coordinator with N EVM TestChain's (Servermint apps) and M Cosmos chains (Simulation Apps)
...
func SendMsgs(chain *ibctesting.TestChain, feeAmt int64, msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	if servermintChain, ok := chain.App.(*app.Servermint); ok {
		bondDenom = servermintChain.StakingKeeper.BondDenom(chain.GetContext())
	} else {
