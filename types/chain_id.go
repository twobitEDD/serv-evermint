...
var (
	...
	regexEpoch           = `[1-9][0-9]*`
	servermintChainID      = regexp.MustCompile(fmt.Sprintf(`^(%s)%s(%s)%s(%s)$`,
		regexChainID,
...
func IsValidChainID(chainID string) bool {
	...
	return servermintChainID.MatchString(chainID)
}
...
func ParseChainID(chainID string) (*big.Int, error) {
	...
	matches := servermintChainID.FindStringSubmatch(chainID)
