...
func TestNevermindExport(t *testing.T) {
	// create public key
	privVal := mock.NewPV()
	...
	db := dbm.NewMemDB()
	chainApp := NewNevermind(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, encoding.MakeConfig(ModuleBasics), simapp.EmptyAppOptions{})
	...
	app2 := NewNevermind(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, encoding.MakeConfig(ModuleBasics), simapp.EmptyAppOptions{})
