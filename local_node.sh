...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	for KEY in "${KEYS[@]}"; do
		"$BINARY" add-genesis-account "$KEY" 100000000000000000000000000wei --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	"$BINARY" gentx "${KEYS[0]}" 1000000000000000000000wei --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
...
"$BINARY" start --metrics "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001wei --json-rpc.api eth,txpool,personal,net,debug,web3 --api.enable --home "$HOMEDIR"
