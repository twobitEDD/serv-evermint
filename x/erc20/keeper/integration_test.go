...
func submitRegisterCoinProposal(ctx sdk.Context, chainApp *app.Servermint, pk *ethsecp256k1.PrivKey, metadata []banktypes.Metadata) (id uint64, err error) {
	content := types.NewRegisterCoinProposal("test Coin", "foo", metadata...)
	return testutil.SubmitProposal(ctx, chainApp, pk, content, 8)
}

func submitRegisterERC20Proposal(ctx sdk.Context, chainApp *app.Servermint, pk *ethsecp256k1.PrivKey, addrs []string) (id uint64, err error) {
	content := types.NewRegisterERC20Proposal("test token", "foo", addrs...)
	return testutil.SubmitProposal(ctx, chainApp, pk, content, 8)
}

func convertCoin(ctx sdk.Context, chainApp *app.Servermint, pk *ethsecp256k1.PrivKey, coin sdk.Coin) {
...
func convertERC20(ctx sdk.Context, chainApp *app.Servermint, pk *ethsecp256k1.PrivKey, amt math.Int, contract common.Address) {
	addrBz := pk.PubKey().Address().Bytes()
