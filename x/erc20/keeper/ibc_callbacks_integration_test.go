...
import (
	...
	"github.com/VictorTrustyDev/nevermind/v12/contracts"
...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	nativeCoinMeta := banktypes.Metadata{
		Description: "Base Denom for Nevermind Chain",
	...
	Describe("disabled params", func() {
		BeforeEach(func() {
			...
			err := s.app.Erc20Keeper.SetParams(s.NevermindChain.GetContext(), erc20params)
			s.Require().NoError(err)

			sender = s.IBCOsmosisChain.SenderAccount.GetAddress().String()
			receiver = s.NevermindChain.SenderAccount.GetAddress().String()
			receiverAcc = sdk.MustAccAddressFromBech32(receiver)
		})
		It("should transfer and not convert to erc20", func() {
			// register the pair to check that it was not converted to ERC-20
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.NevermindChain.GetContext(), osmoMeta)
			s.Require().NoError(err)

			// check balance before transfer is 0
			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcOsmoBalanceBefore.Amount.Int64())

			s.SendAndReceiveMessage(s.pathOsmosisNevermind, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// check balance after transfer
			ibcOsmoBalanceAfter := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(amount, ibcOsmoBalanceAfter.Amount.Int64())

			// check ERC20 balance - should be zero (no conversion)
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and convert uosmo to tokens", func() {
			...
			balanceTokenBefore := s.app.Erc20Keeper.BalanceOf(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			s.Require().Equal(int64(0), balanceTokenBefore.Int64())

			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcOsmoBalanceBefore.Amount.Int64())

			s.NevermindChain.Coordinator.CommitBlock()
			// Send coins
			s.SendAndReceiveMessage(s.pathOsmosisNevermind, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// Check ERC20 balances
			balanceTokenAfter := s.app.Erc20Keeper.BalanceOf(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
		...
		It("should transfer and not convert unregistered coin (uatom)", func() {
			...
			// check balance before transfer is 0
			ibcAtomBalanceBefore := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, teststypes.UatomIbcdenom)
			s.Require().Equal(int64(0), ibcAtomBalanceBefore.Amount.Int64())

			s.NevermindChain.Coordinator.CommitBlock()
			s.SendAndReceiveMessage(s.pathCosmosNevermind, s.IBCCosmosChain, "uatom", amount, sender, receiver, 1, "")

			// check balance after transfer
			ibcAtomBalanceAfter := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, teststypes.UatomIbcdenom)
		...
		It("should transfer and not convert "+constants.BaseDenom, func() {
			...
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.NevermindChain.GetContext(), nativeCoinMeta)
			s.Require().NoError(err)

			nativeCoinInitialBalance := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, constants.BaseDenom)

			// 1. Send native coin from Nevermind to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisNevermind, s.NevermindChain, constants.BaseDenom, amount, receiver, sender, 1, "")

			nativeCoinAfterBalance := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, constants.BaseDenom)
			...
			// 2. Send native coin as IBC coins from Osmosis to Nevermind
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.NativeCoinDenomtrace.Path, teststypes.NativeCoinDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisNevermind, s.IBCOsmosisChain, teststypes.NativeCoinIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)
			...
			nativeCoinFinalBalance := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, constants.BaseDenom)
			...
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, teststypes.NativeCoinIbcdenom)
		...
		It("should transfer and convert original erc20", func() {
			...
			// 1. Send 'uosmo' from Osmosis to Nevermind
			s.SendAndReceiveMessage(s.pathOsmosisNevermind, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// validate 'uosmo' was transferred successfully and converted to ERC20
			balanceERC20Token := s.app.Erc20Keeper.BalanceOf(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			s.Require().Equal(amount, balanceERC20Token.Int64())

			// 2. Transfer back the erc20 from Nevermind to Osmosis
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.UosmoDenomtrace.Path, teststypes.UosmoDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisNevermind, s.NevermindChain, types.ModuleName+"/"+pair.GetERC20Contract().String(), amount, receiver, sender, 1, ibcCoinMeta)
			...
			// check IBC Coin balance - should be zero
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcCoinsBalance.Amount.Int64())
	...
	Describe("Performing recovery with registered coin", func() {
		BeforeEach(func() {
			...
			// but funds are transferred to this address in Nevermind chain
			receiver = sender
			senderAcc = sdk.MustAccAddressFromBech32(sender)
			receiverAcc = sdk.MustAccAddressFromBech32(receiver)
		...
		It("should recover and not convert uosmo to tokens", func() {
			...
			// Send 'uosmo' to Osmosis address in Nevermind Chain (locked funds)
			// sender_addr == receiver_addr
			s.SendAndReceiveMessage(s.pathOsmosisNevermind, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")
			timeout := uint64(s.NevermindChain.GetContext().BlockTime().Add(time.Hour * 4).Add(time.Second * -20).UnixNano())
			err := s.pathOsmosisNevermind.RelayPacket(CreatePacket("10", "transfer/channel-0/uosmo", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
			s.Require().NoError(err)
	...
	Describe("Performing claims with registered coin", func() {
		BeforeEach(func() {
			s.app.Erc20Keeper.SetParams(s.NevermindChain.GetContext(), types.DefaultParams()) //nolint:errcheck
			...
			// Authorize channel-0 for claims (Nevermind-Osmosis)
			params := s.app.ClaimsKeeper.GetParams(s.NevermindChain.GetContext())
			params.AuthorizedChannels = []string{
				"channel-0",
			}
			s.app.ClaimsKeeper.SetParams(s.NevermindChain.GetContext(), params) //nolint:errcheck
		})
		It("it should perform the claim and convert the received tokens", func() {
			...
			s.app.ClaimsKeeper.SetClaimsRecord(s.NevermindChain.GetContext(), senderAcc, claimstypes.ClaimsRecord{
			...
			err := testutil.FundModuleAccount(s.NevermindChain.GetContext(), s.app.BankKeeper, claimstypes.ModuleName, coins)
			s.Require().NoError(err)

			receiverInitialNativeCoinBalance := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, constants.BaseDenom)

			uosmoInitialBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, "uosmo")

			// Send 'uosmo' from Osmosis address with claims to Nevermind address
			...
			// validate that Receiver address on Nevermind got the claims tokens
			receiverFinalNativeCoinBalance := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), receiverAcc, constants.BaseDenom)
	...
	Describe("registered erc20", func() {
		BeforeEach(func() { //nolint:dupl
			...
			sender = s.NevermindChain.SenderAccount.GetAddress().String()
			receiverAcc = sdk.MustAccAddressFromBech32(receiver)
			senderAcc = sdk.MustAccAddressFromBech32(sender)
			...
			s.Require().NoError(err)
			pair, err = s.app.Erc20Keeper.RegisterERC20(s.NevermindChain.GetContext(), addr)
			s.Require().NoError(err)
			...
			s.NevermindChain.SenderAccount.SetSequence(s.NevermindChain.SenderAccount.GetSequence() + 1) //nolint:errcheck
		})
		It("should convert erc20 ibc voucher to original erc20", func() {
			// Mint tokens and send to receiver
			_, err := s.app.Erc20Keeper.CallEVM(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, common.BytesToAddress(senderAcc.Bytes()), pair.GetERC20Contract(), true, "mint", common.BytesToAddress(senderAcc.Bytes()), big.NewInt(amount))
			s.Require().NoError(err)
			// Check Balance
			balanceToken := s.app.Erc20Keeper.BalanceOf(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			...
			_, err = s.app.Erc20Keeper.ConvertERC20(sdk.WrapSDKContext(s.NevermindChain.GetContext()), msgConvertERC20)
			s.Require().NoError(err)
			...
			erc20CoinsBalance := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), senderAcc, pair.Denom)
			s.Require().Equal(amount, erc20CoinsBalance.Amount.Int64())

			s.NevermindChain.Coordinator.CommitBlock()

			// Attempt to send erc20 into ibc, should send without conversion
			s.SendBackCoins(s.pathOsmosisNevermind, s.NevermindChain, pair.Denom, amount, sender, receiver, 1, pair.Denom)
			...
			s.SendAndReceiveMessage(s.pathOsmosisNevermind, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
		})
		It("should convert full available balance of erc20 coin to original erc20 token", func() {
			...
			// Attempt to send erc20 into ibc, should send without conversion
			s.SendBackCoins(s.pathOsmosisNevermind, s.NevermindChain, pair.Denom, amount/2, sender, receiver, 1, pair.Denom)
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			...
			s.SendAndReceiveMessage(s.pathOsmosisNevermind, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount/2, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
		...
		It("send native ERC-20 to osmosis, when sending back IBC coins should convert full balance back to erc20 token", func() {
			...
			s.SendBackCoins(s.pathOsmosisNevermind, s.NevermindChain, types.ModuleName+"/"+pair.GetERC20Contract().String(), amount/2, sender, receiver, 1, "")
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			...
			// send back the IBC coins from Osmosis to Nevermind
			s.SendAndReceiveMessage(s.pathOsmosisNevermind, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount/2, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
...
var _ = Describe("Convert outgoing ERC20 to IBC", Ordered, func() {
	...
	Describe("disabled params", func() {
		...
		It("should fail transfer and not convert to IBC", func() {
			...
			denom := originChain.App.(*app.Nevermind).StakingKeeper.BondDenom(originChain.GetContext())
	...
	Describe("registered erc20", func() {
		...
		It("should transfer available balance", func() {
			// Mint tokens and send to receiver
			_, err := s.app.Erc20Keeper.CallEVM(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, common.BytesToAddress(senderAcc.Bytes()), pair.GetERC20Contract(), true, "mint", common.BytesToAddress(senderAcc.Bytes()), big.NewInt(amount*2))
			s.Require().NoError(err)
			// Check Balance
			balanceToken := s.app.Erc20Keeper.BalanceOf(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount*2, balanceToken.Int64())
	...
	Describe("registered coin", func() {
		BeforeEach(func() {
			...
			// Send from osmosis to Nevermind
			s.SendAndReceiveMessage(s.pathOsmosisNevermind, s.IBCOsmosisChain, "uosmo", amount, receiver, sender, 1, "")
			s.NevermindChain.Coordinator.CommitBlock(s.NevermindChain)
			erc20params.EnableErc20 = true
			err = s.app.Erc20Keeper.SetParams(s.NevermindChain.GetContext(), erc20params)
			s.Require().NoError(err)
		...
		It("should convert erc20 to ibc vouched and transfer", func() {
			...
			balance := s.app.BankKeeper.GetBalance(s.NevermindChain.GetContext(), senderAcc, teststypes.UosmoIbcdenom)
			...
			s.Require().NoError(err)
			// Use MsgConvertERC20 to convert the ERC20 to a Cosmos IBC Coin
			_, err = s.app.Erc20Keeper.ConvertCoin(sdk.WrapSDKContext(s.NevermindChain.GetContext()), msgConvertCoin)
			s.Require().NoError(err)

			s.NevermindChain.Coordinator.CommitBlock()

			// Attempt to send erc20 tokens to osmosis and convert automatically
			s.SendBackCoins(s.pathOsmosisNevermind, s.NevermindChain, pair.Denom, amount, sender, receiver, 1, teststypes.UosmoDenomtrace.GetFullDenomPath())
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			// Check balance on the Osmosis chain
		...
		It("should timeout and reconvert coins", func() {
			...
			originChain := s.NevermindChain
			coin := pair.Denom
			currentTime := s.NevermindChain.Coordinator.CurrentTime
			...
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.NevermindChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(int64(0), balanceERC20TokenAfter.Int64())
