...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	nativeCoinMeta := banktypes.Metadata{
		Description: "Base Denom for Servermint Chain",
	...
	Describe("disabled params", func() {
		BeforeEach(func() {
			...
			err := s.app.Erc20Keeper.SetParams(s.ServermintChain.GetContext(), erc20params)
			s.Require().NoError(err)

			sender = s.IBCOsmosisChain.SenderAccount.GetAddress().String()
			receiver = s.ServermintChain.SenderAccount.GetAddress().String()
			receiverAcc = sdk.MustAccAddressFromBech32(receiver)
		})
		It("should transfer and not convert to erc20", func() {
			// register the pair to check that it was not converted to ERC-20
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.ServermintChain.GetContext(), osmoMeta)
			s.Require().NoError(err)

			// check balance before transfer is 0
			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcOsmoBalanceBefore.Amount.Int64())

			s.SendAndReceiveMessage(s.pathOsmosisServermint, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// check balance after transfer
			ibcOsmoBalanceAfter := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(amount, ibcOsmoBalanceAfter.Amount.Int64())

			// check ERC20 balance - should be zero (no conversion)
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and convert uosmo to tokens", func() {
			...
			balanceTokenBefore := s.app.Erc20Keeper.BalanceOf(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			s.Require().Equal(int64(0), balanceTokenBefore.Int64())

			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcOsmoBalanceBefore.Amount.Int64())

			s.ServermintChain.Coordinator.CommitBlock()
			// Send coins
			s.SendAndReceiveMessage(s.pathOsmosisServermint, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// Check ERC20 balances
			balanceTokenAfter := s.app.Erc20Keeper.BalanceOf(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
		...
		It("should transfer and not convert unregistered coin (uatom)", func() {
			...
			// check balance before transfer is 0
			ibcAtomBalanceBefore := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, teststypes.UatomIbcdenom)
			s.Require().Equal(int64(0), ibcAtomBalanceBefore.Amount.Int64())

			s.ServermintChain.Coordinator.CommitBlock()
			s.SendAndReceiveMessage(s.pathCosmosServermint, s.IBCCosmosChain, "uatom", amount, sender, receiver, 1, "")

			// check balance after transfer
			ibcAtomBalanceAfter := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, teststypes.UatomIbcdenom)
		...
		It("should transfer and not convert "+constants.BaseDenom, func() {
			...
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.ServermintChain.GetContext(), nativeCoinMeta)
			s.Require().NoError(err)

			nativeCoinInitialBalance := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, constants.BaseDenom)

			// 1. Send native coin from Servermint to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisServermint, s.ServermintChain, constants.BaseDenom, amount, receiver, sender, 1, "")

			nativeCoinAfterBalance := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, constants.BaseDenom)
			...
			// 2. Send native coin as IBC coins from Osmosis to Servermint
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.NativeCoinDenomtrace.Path, teststypes.NativeCoinDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisServermint, s.IBCOsmosisChain, teststypes.NativeCoinIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)
			...
			nativeCoinFinalBalance := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, constants.BaseDenom)
			...
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, teststypes.NativeCoinIbcdenom)
		...
		It("should transfer and convert original erc20", func() {
			...
			// 1. Send 'uosmo' from Osmosis to Servermint
			s.SendAndReceiveMessage(s.pathOsmosisServermint, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// validate 'uosmo' was transferred successfully and converted to ERC20
			balanceERC20Token := s.app.Erc20Keeper.BalanceOf(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			s.Require().Equal(amount, balanceERC20Token.Int64())

			// 2. Transfer back the erc20 from Servermint to Osmosis
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.UosmoDenomtrace.Path, teststypes.UosmoDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisServermint, s.ServermintChain, types.ModuleName+"/"+pair.GetERC20Contract().String(), amount, receiver, sender, 1, ibcCoinMeta)
			...
			// check IBC Coin balance - should be zero
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcCoinsBalance.Amount.Int64())
	...
	Describe("Performing claims with registered coin", func() {
		BeforeEach(func() {
			s.app.Erc20Keeper.SetParams(s.ServermintChain.GetContext(), types.DefaultParams()) //nolint:errcheck

			sender = s.IBCOsmosisChain.SenderAccount.GetAddress().String()
			// receiver address is on Osmosis Chain also,
			// but funds are transferred to this address in Servermint chain
			receiver = s.ServermintChain.SenderAccount.GetAddress().String()
			senderAcc = sdk.MustAccAddressFromBech32(sender)
			receiverAcc = sdk.MustAccAddressFromBech32(receiver)
			...
			// Authorize channel-0 for claims (Servermint-Osmosis)
			params := s.app.ClaimsKeeper.GetParams(s.ServermintChain.GetContext())
			params.AuthorizedChannels = []string{
				"channel-0",
			}
			s.app.ClaimsKeeper.SetParams(s.ServermintChain.GetContext(), params) //nolint:errcheck
		})
		It("it should perform the claim and convert the received tokens", func() {
			...
			s.app.ClaimsKeeper.SetClaimsRecord(s.ServermintChain.GetContext(), senderAcc, claimstypes.ClaimsRecord{
			...
			err := testutil.FundModuleAccount(s.ServermintChain.GetContext(), s.app.BankKeeper, claimstypes.ModuleName, coins)
			s.Require().NoError(err)

			receiverInitialNativeCoinBalance := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, constants.BaseDenom)

			uosmoInitialBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, "uosmo")

			// Send 'uosmo' from Osmosis address with claims to Servermint address
			...
			// validate that Receiver address on Servermint got the claims tokens
			receiverFinalNativeCoinBalance := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), receiverAcc, constants.BaseDenom)
	...
	Describe("registered erc20", func() {
		BeforeEach(func() { //nolint:dupl
			...
			sender = s.ServermintChain.SenderAccount.GetAddress().String()
			receiverAcc = sdk.MustAccAddressFromBech32(receiver)
			senderAcc = sdk.MustAccAddressFromBech32(sender)
			...
			s.Require().NoError(err)
			pair, err = s.app.Erc20Keeper.RegisterERC20(s.ServermintChain.GetContext(), addr)
			s.Require().NoError(err)
			...
			s.ServermintChain.SenderAccount.SetSequence(s.ServermintChain.SenderAccount.GetSequence() + 1) //nolint:errcheck
		})
		It("should convert erc20 ibc voucher to original erc20", func() {
			// Mint tokens and send to receiver
			_, err := s.app.Erc20Keeper.CallEVM(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, common.BytesToAddress(senderAcc.Bytes()), pair.GetERC20Contract(), true, "mint", common.BytesToAddress(senderAcc.Bytes()), big.NewInt(amount))
			s.Require().NoError(err)
			// Check Balance
			balanceToken := s.app.Erc20Keeper.BalanceOf(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			...
			_, err = s.app.Erc20Keeper.ConvertERC20(sdk.WrapSDKContext(s.ServermintChain.GetContext()), msgConvertERC20)
			s.Require().NoError(err)
			...
			erc20CoinsBalance := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), senderAcc, pair.Denom)
			s.Require().Equal(amount, erc20CoinsBalance.Amount.Int64())

			s.ServermintChain.Coordinator.CommitBlock()

			// Attempt to send erc20 into ibc, should send without conversion
			s.SendBackCoins(s.pathOsmosisServermint, s.ServermintChain, pair.Denom, amount, sender, receiver, 1, pair.Denom)
			...
			s.SendAndReceiveMessage(s.pathOsmosisServermint, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
		})
		It("should convert full available balance of erc20 coin to original erc20 token", func() {
			...
			// Attempt to send erc20 into ibc, should send without conversion
			s.SendBackCoins(s.pathOsmosisServermint, s.ServermintChain, pair.Denom, amount/2, sender, receiver, 1, pair.Denom)
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			...
			s.SendAndReceiveMessage(s.pathOsmosisServermint, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount/2, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
		...
		It("send native ERC-20 to osmosis, when sending back IBC coins should convert full balance back to erc20 token", func() {
			...
			s.SendBackCoins(s.pathOsmosisServermint, s.ServermintChain, types.ModuleName+"/"+pair.GetERC20Contract().String(), amount/2, sender, receiver, 1, "")
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			...
			// send back the IBC coins from Osmosis to Servermint
			s.SendAndReceiveMessage(s.pathOsmosisServermint, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount/2, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
...
var _ = Describe("Convert outgoing ERC20 to IBC", Ordered, func() {
	...
	Describe("disabled params", func() {
		...
		It("should fail transfer and not convert to IBC", func() {
			...
			path := s.pathOsmosisServermint
			originEndpoint := path.EndpointB
			destEndpoint := path.EndpointA
			originChain := s.ServermintChain
			coin := pair.Denom
			...
			denom := originChain.App.(*app.Servermint).StakingKeeper.BondDenom(originChain.GetContext())
	...
	Describe("registered erc20", func() {
		...
		It("should transfer available balance", func() {
			// Mint tokens and send to receiver
			_, err := s.app.Erc20Keeper.CallEVM(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, common.BytesToAddress(senderAcc.Bytes()), pair.GetERC20Contract(), true, "mint", common.BytesToAddress(senderAcc.Bytes()), big.NewInt(amount*2))
			s.Require().NoError(err)
			// Check Balance
			balanceToken := s.app.Erc20Keeper.BalanceOf(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount*2, balanceToken.Int64())
	...
	Describe("registered coin", func() {
		BeforeEach(func() {
			...
			// Send from osmosis to Servermint
			s.SendAndReceiveMessage(s.pathOsmosisServermint, s.IBCOsmosisChain, "uosmo", amount, receiver, sender, 1, "")
			s.ServermintChain.Coordinator.CommitBlock(s.ServermintChain)
			erc20params.EnableErc20 = true
			err = s.app.Erc20Keeper.SetParams(s.ServermintChain.GetContext(), erc20params)
			s.Require().NoError(err)
		...
		It("should convert erc20 to ibc vouched and transfer", func() {
			...
			balance := s.app.BankKeeper.GetBalance(s.ServermintChain.GetContext(), senderAcc, teststypes.UosmoIbcdenom)
			...
			s.Require().NoError(err)
			// Use MsgConvertERC20 to convert the ERC20 to a Cosmos IBC Coin
			_, err = s.app.Erc20Keeper.ConvertCoin(sdk.WrapSDKContext(s.ServermintChain.GetContext()), msgConvertCoin)
			s.Require().NoError(err)

			s.ServermintChain.Coordinator.CommitBlock()

			// Attempt to send erc20 tokens to osmosis and convert automatically
			s.SendBackCoins(s.pathOsmosisServermint, s.ServermintChain, pair.Denom, amount, sender, receiver, 1, teststypes.UosmoDenomtrace.GetFullDenomPath())
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			// Check balance on the Osmosis chain
		...
		It("should timeout and reconvert coins", func() {
			...
			originChain := s.ServermintChain
			coin := pair.Denom
			currentTime := s.ServermintChain.Coordinator.CurrentTime
			...
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.ServermintChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(int64(0), balanceERC20TokenAfter.Int64())
