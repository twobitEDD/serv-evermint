...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	s.app = suite.NevermindChain.App.(*app.Nevermind)
	evmParams := s.app.EvmKeeper.GetParams(s.NevermindChain.GetContext())
	evmParams.EvmDenom = constants.BaseDenom
	err := s.app.EvmKeeper.SetParams(s.NevermindChain.GetContext(), evmParams)
	suite.Require().NoError(err)

	// s.app.FeeMarketKeeper.SetBaseFee(s.NevermindChain.GetContext(), big.NewInt(1))
	...
	validators := s.app.StakingKeeper.GetValidators(suite.NevermindChain.GetContext(), 2)
	cons, err := validators[0].GetConsAddr()
	suite.Require().NoError(err)
	suite.NevermindChain.CurrentHeader.ProposerAddress = cons.Bytes()

	err = s.app.StakingKeeper.SetValidatorByConsAddr(suite.NevermindChain.GetContext(), validators[0])
	suite.Require().NoError(err)

	_, err = s.app.EvmKeeper.GetCoinbaseAddress(suite.NevermindChain.GetContext(), sdk.ConsAddress(suite.NevermindChain.CurrentHeader.ProposerAddress))
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(suite.NevermindChain.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToAccount(suite.NevermindChain.GetContext(), inflationtypes.ModuleName, suite.NevermindChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(s.NevermindChain.GetContext(), types.ModuleName, coins)
	...
	err = s.app.ClaimsKeeper.SetParams(suite.NevermindChain.GetContext(), claimparams)
	suite.Require().NoError(err)

	params := types.DefaultParams()
	params.EnableErc20 = true
	err = s.app.Erc20Keeper.SetParams(suite.NevermindChain.GetContext(), params)
	suite.Require().NoError(err)
	...
	coins = sdk.NewCoins(nativeCoin)
	err = s.app.BankKeeper.MintCoins(suite.NevermindChain.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(suite.NevermindChain.GetContext(), types.ModuleName, authtypes.FeeCollectorName, coins)
	suite.Require().NoError(err)
}
...
// to the Nevermind chain (used on IBC tests)
func (suite *KeeperTestSuite) DeployContractToChain(name, symbol string, decimals uint8) (common.Address, error) {
	return testutil.DeployContract(
		s.NevermindChain.GetContext(),
		s.NevermindChain.App.(*app.Nevermind),
		suite.NevermindChain.SenderPrivKey,
		suite.queryClientEvm,
		contracts.ERC20MinterBurnerDecimalsContract,
		name, symbol, decimals,
	)
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/uever") as input parameter.
